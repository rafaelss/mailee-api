  ...
  module Sync
    ...
    module ClassMethods
      def sync_with_mailee(options = {})
        ...
        unless self.column_names.include?(self.sync_options[:email].to_s)
          raise "Campo #{sync_options[:email]} não existe em #{self}."
        end
    ...
    module InstanceMethods #:nodoc:
      def create_in_mailee
        return if sync_options[:news] && !send(sync_options[:news]) # Não cria se houver o campo booleano e ele for falso
